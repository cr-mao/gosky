...
func init() {
	//日志相关配置
	conf.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			"level": conf.Env("LOG_LEVEL", "debug"),
			// 日志的类型，可选：
			// "single" 独立的文件
			// "daily" 按照日期每日一个
			"type": conf.Env("LOG_TYPE", "daily"),
			/* ------------------ 滚动日志配置 ------------------ */
			// 日志文件路径
			"filename": conf.Env("LOG_NAME", "storage/logs/logs.log"),
			// 每个日志文件保存的最大尺寸 单位：M
			"max_size": conf.Env("LOG_MAX_SIZE", 64),
			// 最多保存日志文件数，0 为不限，MaxAge 到了还是会删
			"max_backup": conf.Env("LOG_MAX_BACKUP", 30),
			// 最多保存多少天，7 表示一周前的日志会被删除，0 表示不删
			"max_age": conf.Env("LOG_MAX_AGE", 7),
			// 是否压缩，压缩日志不方便查看，我们设置为 false（压缩可节省空间）
			"compress": conf.Env("LOG_COMPRESS", false),
		}
	})
}
